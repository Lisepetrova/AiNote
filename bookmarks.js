...
async function processVideo(bookmarkId, userId, url) {
  try {
    ...
    // YouTube: НЕ трогаем yt-dlp (его блокирует YouTube с серверных IP).
    // Ссылку забирает сам Gemini напрямую. Метаданные не нужны.
    let meta;
    let localFile = null;
    if (platform === 'youtube') {
      meta = { platform, title: '', description: '', uploader: '' };
    } else {
